...
$console->registerModules([
    'cli' => ['className' => 'Route\Modules\Cli\Module']
]);
...
try {
    ...
    if (!file_exists(BASE_PATH . "/log/" . date('Y-m-d') . "_error.log")) {
        file_put_contents(BASE_PATH . "/log/" . date('Y-m-d') . "_error.log", '');
    }
    $logger = new \Phalcon\Logger\Adapter\File(BASE_PATH."/log/".date('Y-m-d')."_error.log");
    $logger->log(PHP_EOL.$e->getMessage().$e->getTraceAsString(),\Phalcon\Logger::ERROR);
